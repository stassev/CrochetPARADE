...
var exportSVGinColor=false;
    ...
    var str = JSON.parse(JSON.stringify(json0)); //JSON.parse59.json0);
    console.log(str);
    
// 1) Small sample of objects
console.log("=== str.objects sample ===");
//console.log(str.objects.slice(0, 5));

// 2) Small sample of edges, with head/tail labels resolved
console.log("=== str.edges sample (with head/tail labels) ===");
//console.log(
//  str.edges.slice(0, 5).map(e => {
//    const headObj = str.objects.find(o => o._gvid === e.head);
//    const tailObj = str.objects.find(o => o._gvid === e.tail);
//    return {
//      edgeLabel: e.label,        // raw edge label
//      edgeColor: e.color,
//      headId: e.head,
//      tailId: e.tail,
//      headLabel: headObj && headObj.label,
//      tailLabel: tailObj && tailObj.label,
//      headName: headObj && headObj.name,
//      tailName: tailObj && tailObj.name
//    };
//  })
//);
...
// Some node/edge ids can contain multiple stitch numbers, e.g. "20,7|391a21,1|396_jacobian0.2".
// For highlighting and selection, we want the *canonical* stitch number for that id.
// Empirically in this codebase, the correct choice is the *last* "|<stnum>" occurrence.
function extractLastPipeNumber(name) {
    try {
        const re = /\|(\d+)/g;
        let m, last = null;
...
str.objects.forEach((obj) => {
    ...
    if (obj.label.split('|')[0] !== "hidden") {
        ...
        node['type'] = 0;
        node['stnum'] = extractLastPipeNumber(obj.name);
        if (node['stnum'] == null || isNaN(node['stnum'])) node['stnum'] = parseInt(obj.name.split('|')[1]);
        node['id0'] = obj.name.split('|')[0];

        node['dot_id'] = obj.name;
...
// --------------------------------------------------------------------
// Highlight material helper (reads window.__HL_COLORS).
// Ensures consistent colors for different highlight mechanisms across all 3D objects
// (edges, arrowheads, spheres, etc.), choosing line-vs-mesh material automatically.
// --------------------------------------------------------------------
const __HL_MAT_CACHE = new Map();
function __hlColorFor(src) {
    try {
        return (typeof window !== 'undefined' && window.__HL_COLORS && window.__HL_COLORS[src]) ? window.__HL_COLORS[src] : null;
    } catch (e) {}
    return null;
}
function __hlMat(obj, src) {
    const hex = __hlColorFor(src) || '#ffffff';
    const mat = obj ? obj.material : null;
    const isLine = !!(obj && (obj.isLine || obj.isLineSegments)) ||
        !!(mat && (mat.isLineBasicMaterial || mat.isLineDashedMaterial));
    const kind = isLine ? 'line' : 'mesh';
    const key = kind + '|' + hex;
    const cached = __HL_MAT_CACHE.get(key);
    if (cached) return cached;

    let m;
    if (kind === 'line') {
        m = new THREE.LineBasicMaterial({ color: new THREE.Color(hex), linewidth: 2 });
    } else {
        m = new THREE.MeshBasicMaterial({ color: new THREE.Color(hex) });
    }
    __HL_MAT_CACHE.set(key, m);
    return m;
}
// --------------------------------------------------------------------
// Unified 3D highlight manager
//
// Several independent highlight mechanisms (hover, Ctrl-click selection in canvas,
// Ctrl-click selection from expanded text, periphery highlight) historically toggled
// materials directly and each kept its own "old material" stacks. That caused
// clashes and stepping when multiple mechanisms overlapped.
//
// This manager provides per-object layered highlights with explicit precedence:
//   selection (canvas/text, most-recent wins) > periphery > hover > base
// Base materials are updated on global mode changes (e.g. 'c' / Escape).
// --------------------------------------------------------------------
const __HL3D = (() => {
    // Per-object state: { base, layers: Map<src, {mat, epoch}> }
    const st = new WeakMap();
    // Track objects per source so we can clear a source efficiently.
    const srcSets = {
        hover: new Set(),
        periphery: new Set(),
        ctrlD: new Set(),
        ctrlF: new Set(),
        selectCanvas: new Set(),
        selectText: new Set(),
    };
    // Global epoch: latest mechanism wins per object.
    let epoch = 1;

    function ensure(obj) {
        let s = st.get(obj);
        if (!s) {
            s = { base: obj.material, layers: new Map() };
            st.set(obj, s);
        }
        return s;
    }

    function bestLayer(s) {
        let best = null;
        for (const v of s.layers.values()) {
            if (v && (!best || v.epoch > best.epoch)) best = v;
        }
        return best ? best.mat : s.base;
    }

    function apply(obj, s) {
        const mat = bestLayer(s);
        if (obj.material !== mat) obj.material = mat;
    }

    function setBase(obj, mat) {
        const s = ensure(obj);
        s.base = mat;
        apply(obj, s);
    }

    function add(obj, src, mat) {
        const s = ensure(obj);
        // If no layers were active, capture current material as base so we restore correctly.
        if (s.layers.size === 0) s.base = obj.material;
        s.layers.set(src, { mat, epoch: epoch++ });
        if (srcSets[src]) srcSets[src].add(obj);
        if (src === 'periphery') obj.__peripheryHighlighted = true;
        apply(obj, s);
    }

    function remove(obj, src) {
        const s = st.get(obj);
        if (!s || !s.layers.has(src)) return;
        s.layers.delete(src);
        if (srcSets[src]) srcSets[src].delete(obj);
        if (src === 'periphery') delete obj.__peripheryHighlighted;
        apply(obj, s);
    }

    function clearSource(src) {
        const set = srcSets[src];
        if (!set) return;
        for (const obj of Array.from(set)) {
            try { remove(obj, src); } catch (e) {}
        }
        set.clear();
    }
    ...
    function clearAll() {
        clearSource('hover');
        clearSource('selectCanvas');
        clearSource('selectText');
        clearSource('ctrlD');
        clearSource('ctrlF');
        clearSource('periphery');
    }

    return { setBase, add, remove, clearSource, clearAll };
})();var stLen = {};
var stTot = {};
...
//// Create the edges
// Map from "tailStnum->headStnum" to the actual 3D edge meshes (cylinders) and their arrowheads.
// This lets periphery highlighting target the *specific* periphery edges, instead of every edge incident on a node.
const edgeStnumPairDictionary = new Map();
// Map from \"tailName\u0000headName\" to the actual 3D edge meshes (cylinders) and their arrowheads.
// This is more precise than stnum-based lookup for composite node ids.
const edgeNamePairDictionary = new Map();
...
str.edges.forEach((edge) => {
    ...
    const tail = str.objects.find((obj) => obj._gvid === edge.tail);
    const head = str.objects.find((obj) => obj._gvid === edge.head);
    let tailStnum = extractLastPipeNumber(tail.name);
    if (tailStnum == null || isNaN(tailStnum)) tailStnum = parseInt(tail.name.split('|')[1]);
    ...
    let stnum = extractLastPipeNumber(head.name);
    if (stnum == null || isNaN(stnum)) stnum = parseInt(head.name.split('|')[1]);
...
//console.log('aaa',row)
    ...
    line['tailName'] = tail.name;
    line['headName'] = head.name;
    line['row'] = row;
    ...
    if (!non) {
        ...
        arrowhead['type'] = 2;
        arrowhead['tailName'] = tail.name;
        arrowhead['headName'] = head.name;
    ...
    // Register this specific (tailStnum -> headStnum) edge so periphery highlighting can
    // highlight exactly the periphery edge(s), not every edge that shares a node's stnum.
    const __edgeKey = `${tailStnum}->${stnum}`;
    if (!edgeStnumPairDictionary.has(__edgeKey)) edgeStnumPairDictionary.set(__edgeKey, []);
    edgeStnumPairDictionary.get(__edgeKey).push({ line, arrowhead });
    const __edgeNameKey = `${tail.name}\u0000${head.name}`;
    if (!edgeNamePairDictionary.has(__edgeNameKey)) edgeNamePairDictionary.set(__edgeNameKey, []);
    edgeNamePairDictionary.get(__edgeNameKey).push({ line, arrowhead });
...
// --------------------------------------------------------------------
// Periphery highlighting support (3D scene)
// --------------------------------------------------------------------
...
/**
 * Extract all stitch numbers (stnum) from a periphery node label.
 *
 * Examples:
 *  "10,8|272" → [272]
 *  "11,0|275a12,8|296_jacobian0.2" → [275, 296]
 */
function extractStnumsFromPeripheryLabel(label) {
  ...
  while ((m = re.exec(label)) !== null) {
    ...
    if (!Number.isNaN(s)) stnums.push(s);
  }
  return stnums;
}

/**
 * Clear periphery highlights (scene) without disturbing hover/selection.
 */
window.clearPeripheryHighlightInScene = function () {
  try { __HL3D.clearSource('periphery'); } catch (e) {}
  try { window.__peripherySceneHighlightedNodeLabels = []; } catch (e) {}
  try { window.__currentPeripheryNodeLabels = []; } catch (e) {}
  try { window.__peripheryHighlightSessionId = null; } catch (e) {}
};

/**
 * Highlight nodes + edges (cylinders + arrowheads) for the selected periphery graph.
 *
 * nodeLabels: array of DOT node labels (strings) from periphery.js
 * edgePairs:  array of DOT edges: [[labelA, labelB], ...]
 */
window.highlightPeripheryInScene = function (nodeLabels, edgePairs) {
  // Clear old periphery highlights
  window.clearPeripheryHighlightInScene();

  // Remember current periphery nodes so the text highlighting can re-apply
  // after the instructions are expanded.
  try { window.__currentPeripheryNodeLabels = Array.isArray(nodeLabels) ? nodeLabels.slice() : []; } catch (e) {}
  try { window.__peripheryHighlightSessionId = (window.__peripheryDialogState && typeof window.__peripheryDialogState.sessionId === 'number') ? window.__peripheryDialogState.sessionId : window.__peripheryHighlightSessionId; } catch (e) {}

  const highlightedNodeLabels = [];

  function hl(obj) {
    if (!obj) return;
    try { __HL3D.add(obj, 'periphery', __hlMat(obj, 'periphery')); }
    catch (e) { obj.material = __hlMat(obj, 'periphery'); }
  }

  // ---- Nodes: highlight spheres.
  // Prefer exact DOT node-id matches. Only fall back to stitch-number lookup for
  // *simple* ids (e.g. "18,3|363"). This prevents composite/hidden ids like
  // "18,3|363a19,5|376_jacobian0.2" from incorrectly highlighting visible nodes.
  if (Array.isArray(nodeLabels)) {
    for (const raw of nodeLabels) {
      const label = String(raw ?? '').trim();
      if (!label) continue;

      // Exact match first.
      const exact = (typeof nodeNameDictionary !== 'undefined') ? nodeNameDictionary.get(label) : null;
      if (exact && exact.length) {
        highlightedNodeLabels.push(label);
        for (const obj of exact) {
          if (obj && obj.type === 0 && obj.visible === true) hl(obj);
        }
        continue;
      }

      // Only fall back for simple node ids: "<i>,<j>|<stnum>" (optionally with whitespace).
      if (!/^\s*\d+\s*,\s*\d+\s*\|\s*\d+\s*$/.test(label)) continue;

      const stnum = extractLastPipeNumber(label);
      if (stnum == null) continue;
      const objs = stnumDictionary.get(stnum) || [];
  ...
  // Expose the *actual* node labels that were highlighted, so the text highlighter can avoid
  // hidden/composite nodes.
  try { window.__peripherySceneHighlightedNodeLabels = highlightedNodeLabels; } catch (e) {}

  // ---- Edges + arrowheads: highlight ONLY the edges listed in edgePairs.
  // Do NOT highlight by node stnum (that would also catch unrelated incident edges).
  if (Array.isArray(edgePairs)) {
    for (const pair of edgePairs) {
      if (!Array.isArray(pair) || pair.length < 2) continue;

      const nameA = String(pair[0] ?? '').trim();
      const nameB = String(pair[1] ?? '').trim();

      // Prefer exact edge-id matching (prevents accidental highlighting of adjacent edges
      // when composite node ids share the same leading stitch number).
      if (nameA && nameB) {
        let exact = edgeNamePairDictionary.get(`${nameA}\u0000${nameB}`);
        if (!exact || exact.length === 0) exact = edgeNamePairDictionary.get(`${nameB}\u0000${nameA}`);
        if (exact && exact.length) {
          for (const rec of exact) {
            if (!rec) continue;
            hl(rec.line);
            if (rec.arrowhead) hl(rec.arrowhead);
          }
          continue;
        }
      }

      const stA = extractStnumsFromPeripheryLabel(pair[0]);
      const stB = extractStnumsFromPeripheryLabel(pair[1]);
      if (stA.length === 0 || stB.length === 0) continue;
      // Fallback stnum-based matching is only safe for canonical ids (single stnum each).
      // Composite ids must be matched by exact name (handled above).
      if (stA.length !== 1 || stB.length !== 1) continue;

      for (const a of stA) {
        for (const b of stB) {
          // Try directed key first; if not present, try reverse (some periphery outputs are undirected)
          let matches = edgeStnumPairDictionary.get(`${a}->${b}`);
          if (!matches || matches.length === 0) {
            matches = edgeStnumPairDictionary.get(`${b}->${a}`) || [];
          }
    ...
    // After building stnumDictionary:
console.log("=== NODES summary ===");
console.log("NODES.length =", NODES.length);
if (NODES.length > 0) {
  console.log("Sample NODES[0]:", NODES[0]);
  console.log("Sample NODES[0] key fields:", {
    id0: NODES[0].id0,
    stnum: NODES[0].stnum,
    row: NODES[0].row,
    name: NODES[0].name,
    name_long: NODES[0].name_long
  });
}

// Show a few stnum → nodes mappings
const stKeys = Array.from(stnumDictionary.keys());
console.log("stnumDictionary keys (first 10):", stKeys.slice(0, 10));
if (stKeys.length > 0) {
  const k0 = stKeys[0];
  console.log(`stnumDictionary[${k0}] =`, stnumDictionary.get(k0));
}
    ...
    function highlightSpanById(label,highlightInstructions, source) {
        let inputText = document.getElementById("inputText");
        if (!inputText) return;
        inputText.previousHighlightedIdMesh = inputText.previousHighlightedIdMesh || [];
        ...
        // Extract the node id inside parentheses and derive the canonical stitch number.
        // Some ids contain multiple "|<stnum>" segments (e.g. "...|391a...|396..."); we want the *last* one.
        const nameMatch = topSpanText.match(/\(([^)]*)\)/);
        if (!nameMatch) return; // Exit if no "(...)" portion found

        const rawName = nameMatch[1];
        const stnum = extractLastPipeNumber(rawName);

        if (!Number.isFinite(stnum)) return;

        const formattedString = `^word\\(\\d*\\|\\d*\\)${stnum}$`;
        const regex = new RegExp(formattedString);

        // If requested, clear previous mesh-driven text highlights (but do NOT touch Prism unknown tokens).
        if (highlightInstructions==1){
            if (window.__TextHL && typeof window.__TextHL.clearSource === 'function') {
                window.__TextHL.clearSource(source || 'selectCanvas');
            } else {
        ...
        // Select only word spans inside the Prism-highlighted overlay, to avoid touching syntax error markers.
        const hl = document.getElementById('highlighting');
        if (!hl) return;
        const spans = hl.querySelectorAll('span[id^="word("]');

        for (let span of spans) {
            if (!span || !span.id) continue;
            if (regex.test(span.id)) {
                if (window.__TextHL && typeof window.__TextHL.add === 'function') {
                    window.__TextHL.add(source || 'selectCanvas', span.id);
                } else {
                    span.style.backgroundColor = (window.__HL_COLORS && (window.__HL_COLORS[source || "selectCanvas"])) ? window.__HL_COLORS[source || "selectCanvas"] : "lightgreen";
                }
                inputText.previousHighlightedIdMesh.push(span.id);
            }
        }
      }

    function highlightTextByStnums(stnums, reset, source) {
        const inputText = document.getElementById("inputText");
        if (!inputText || !inputText.expanded) return;

        const hl = document.getElementById('highlighting');
        if (!hl) return;

        const set = (stnums instanceof Set) ? stnums : new Set(stnums || []);

        // Clear previous mesh-driven highlights if requested.
        if (reset) {
            if (window.__TextHL && typeof window.__TextHL.clearSource === 'function') {
                window.__TextHL.clearSource(source || 'selectCanvas');
            } else {
                inputText.previousHighlightedIdMesh = inputText.previousHighlightedIdMesh || [];
                for (const id of inputText.previousHighlightedIdMesh) {
                    if (!id) continue;
                    const el = document.getElementById(id);
                    if (el && el.style && el.style.backgroundColor === 'lightgreen') el.style.backgroundColor = "";
                }
                inputText.previousHighlightedIdMesh = [];
            }
        }
        ...
        // Only word spans (avoid Prism "unknown" syntax markers).
        const spans = hl.querySelectorAll('span[id^="word("]');
        inputText.previousHighlightedIdMesh = inputText.previousHighlightedIdMesh || [];
    ...
    // Robustly apply canvas-origin selection highlight into expanded text.
    // This can race with Prism re-highlighting and ID assignment; retry a few frames if spans aren't ready yet.
    let __selectCanvasTextHLToken = 0;
    function applySelectCanvasTextHighlight(stnum, highlightInstructions) {
        try {
            const inputText = document.getElementById("inputText");
            if (!inputText || !inputText.expanded) return;
            if (!Number.isFinite(stnum)) return;

            const token = ++__selectCanvasTextHLToken;
            const reset = (highlightInstructions === 1);

            const tryApply = (attempt) => {
                if (token !== __selectCanvasTextHLToken) return;

                const hl = document.getElementById('highlighting');
                if (!hl) {
                    if (attempt < 10) requestAnimationFrame(() => tryApply(attempt + 1));
                    return;
                }

                // Ensure IDs exist for spans (overlay may be freshly rebuilt).
                try {
                    if (typeof window.assignIdsAndHover === 'function') window.assignIdsAndHover(hl, false);
                } catch (e) {}
                ...
                if (reset && attempt === 0) {
                    try { if (window.__TextHL) window.__TextHL.clearSource('selectCanvas'); } catch (e) {}
                }

                // Apply highlight by stitch number (reliable; does not depend on label parsing).
                highlightTextByStnums(new Set([stnum]), false, 'selectCanvas');

                // Verify at least one matching span exists; if not, retry (IDs might still be in flux).
                let found = false;
                for (const sp of spans) {
                    const m = sp.id && sp.id.match(/\)(\d+)$/);
                    if (!m) continue;
                    if (parseInt(m[1], 10) === stnum) { found = true; break; }
                }
                if (!found && attempt < 10) requestAnimationFrame(() => tryApply(attempt + 1));
            };
...
var I = null;
    function onMouseDown(event) {
        // Alt+click edge picking for the periphery edge editor (when it's open).
        try {
            if (event && event.button === 0 && event.altKey && event.target === renderer.domElement &&
                window.__peripheryEdgeEditorIsOpen && window.__peripheryEdgeEditorPickEdge &&
                window.__peripheryEdgeEditorIsOpen()) {
                ...
                if (intersects && intersects.length) {
                    for (const inter of intersects) {
                        const o = inter && inter.object;
                        if (o && (o.type === 1 || o.type === 2)) { hitObj = o; break; }
                    }
                }

                if (hitObj && hitObj.tailName && hitObj.headName) {
                    window.__peripheryEdgeEditorPickEdge(hitObj.tailName, hitObj.headName);
        ...
        if ((event.ctrlKey|| event.metaKey) && event.altKey){
            // Additive selection only if we actually clicked on a stitch object.
            let __hit = null;
            try {
                const rect = renderer.domElement.getBoundingClientRect();
                mouse.x = ((event.clientX - rect.left) / rect.width) * 2 - 1;
                mouse.y = -((event.clientY - rect.top) / rect.height) * 2 + 1;
                raycaster.setFromCamera(mouse, camera);
                const __intersects = raycaster.intersectObjects(NODES, true);
                if (__intersects && __intersects.length) {
                    for (const inter of __intersects) {
                        if (inter && inter.object && inter.object.type == 0) { __hit = inter; break; }
                    }
                    if (!__hit) __hit = __intersects[0];
                }
            } catch (e) {}
            if (__hit) {
                I = __hit;
                try { myLabel.innerHTML = "<span style='overflow-wrap: break-word; word-wrap: break-word;'>" + (I.object && I.object.name_long ? I.object.name_long : "") + "</span>"; } catch (e) {}
            ...
            // Replace selection only if we actually clicked on a stitch object.
            let __hit = null;
            try {
                const rect = renderer.domElement.getBoundingClientRect();
                mouse.x = ((event.clientX - rect.left) / rect.width) * 2 - 1;
            ...
            if (__hit) {
                I = __hit;
                try { myLabel.innerHTML = "<span style='overflow-wrap: break-word; word-wrap: break-word;'>" + (I.object && I.object.name_long ? I.object.name_long : "") + "</span>"; } catch (e) {}
                highlightInstructions = (event.altKey ? 2 : 1);
                if (!event.altKey) { try { __HL3D.clearSource('selectCanvas'); } catch (e) {} }
                } else {
                // If nothing was clicked, leave existing highlights unchanged (and allow Ctrl+drag).
                highlightInstructions=0;
            }
        }
               ...
               (!(window.__peripheryEdgeEditorIsOpen &&
                window.__peripheryEdgeEditorIsOpen()))){
           try { if (window.__TextHL) window.__TextHL.clearSource('selectCanvas'); } catch (e) {}
            try { __HL3D.clearSource('selectCanvas'); } catch (e) {}
            highlightInstructions=0;
        }
        else
            highlightInstructions=0;
  ...
  console.log("=== Ctrl+click highlight ===");
  console.log("I.object.id0:", I && I.object && I.object.id0);
  console.log("I.object.stnum:", I && I.object && I.object.stnum);
  console.log("I.object.row:", I && I.object && I.object.row);
  console.log("highlightSpanById label argument (myLabel):", myLabel);
  console.log("highlightInstructions:", highlightInstructions);

            applySelectCanvasTextHighlight((I && I.object) ? I.object.stnum : NaN, highlightInstructions);
            if (I != null && I.object && I.object.row) {
                const keyToFind = `${I.object.row[0]}|${I.object.row[1]}`;
                const matchingNodes = nodeDictionary.get(keyToFind) || [];
                let stnum = (typeof I.object.stnum !== 'undefined') ? I.object.stnum : -1;
                for (const node of matchingNodes) {
                    try { __HL3D.add(node, 'selectCanvas', __hlMat(node, 'selectCanvas')); } catch (e) { node.material = __hlMat(node, 'selectCanvas'); }
                    if (stnum === -1 && node && typeof node.stnum !== 'undefined') stnum = node.stnum;
                }
                if (stnum !== -1) {
                    const matchingNodes1 = stnumDictionary.get(stnum) || [];
...
if ((!inputText.expanded) && (event.altKey)){
    try { if (window.__TextHL && typeof window.__TextHL.clearSource === 'function') { window.__TextHL.clearSource(source || 'selectCanvas'); window.__TextHL.clearSource('selectText'); } } catch (e) {}
    inputText.previousHighlightedIdMesh = [];
    const previouslyHighlightedId = inputText.previousHighlightedIdMesh;
    for (let i of previouslyHighlightedId){
    if (i) {
...
let __hl3dHoverObj = null;

    function isPeripheryCustomEditorOpen() {
        // While the Custom periphery editor is open, hover-highlighting edges (cylinders) can clash with
        // periphery highlighting (hover affects cylinders but not arrowheads). In that mode we suppress
        // hover material changes for edges so periphery edits don't appear to "stick".
        try {
            const modal = document.getElementById('peripheryModal');
            if (!modal) return false;
            if (modal.style && modal.style.display === 'none') return false;
            const det = document.getElementById('periphEditEdgesDetails');
            if (!det) return false;
            if (!det.open) return false;
            // If details is present but hidden (e.g., modal not shown), treat as closed.
            if (det.style && det.style.display === 'none') return false;
            return true;
        } catch (e) {
            return false;
        }
    ...
    const nodeNameDictionary = new Map();
    // Map exact DOT node id (e.g. "18,3|363") to mesh(es). Used to avoid
    // false node highlights for composite/hidden ids like "18,3|363a19,5|376_jacobian0.2".
    for (const i of NODES) {
      const key = (i && (i.dot_id || i.dotId || i.dotID || i.dotid)) ? (i.dot_id || i.dotId || i.dotID || i.dotid) : null;
      if (!key) continue;
      if (!nodeNameDictionary.has(key)) nodeNameDictionary.set(key, []);
      nodeNameDictionary.get(key).push(i);
    }
...
// Unified hover highlighting (layer: hover) -- always active (even when Custom Periphery Editor is open)
if (timeoutQ) {
    try {
        const hoveredObj = (typeof I !== 'undefined' && I && I.object) ? I.object : null;

        // Clear previous hover if different / if pointer left the object.
        if (__hl3dHoverObj && (!hoveredObj || hoveredObj !== __hl3dHoverObj)) {
            try { __HL3D.remove(__hl3dHoverObj, 'hover'); } catch (e) {}
            __hl3dHoverObj = null;
        }

        if (hoveredObj && hoveredObj !== __hl3dHoverObj) {
            __hl3dHoverObj = hoveredObj;
            const mat = __hlMat(hoveredObj, 'hover');
            try { __HL3D.add(hoveredObj, 'hover', mat); } catch (e) { hoveredObj.material = mat; }
        }
    } catch (e) {}
}
    ...
    function handleKeyDown(event) {
        ...
        if (canvasClicked && (event.key === 'c')) {
            setTimeout(function() {
                ...
                for (let i of NODEShidden)
                    i.visible = false;
                // Update base materials for highlight layering (c-mode)
                try {
                    for (let i of NODES) { __HL3D.setBase(i, i.material); }
                    for (let i of NODEShidden) { __HL3D.setBase(i, i.material); }
                } catch (e) {}
        ...
        if (canvasClicked && (event.key === 'd' && (event.ctrlKey || event.metaKey))) {
            ...
            setTimeout(function() {
                ...
                closeButton.onclick = function() {
                    // Close Ctrl+D dialog: remove Ctrl+D highlights (restore previous highlight layers if any).
                    try { __HL3D.clearSource('ctrlD'); } catch (e) {}
                    try { if (window.__TextHL && typeof window.__TextHL.clearSource === 'function') window.__TextHL.clearSource('ctrlD'); } catch (e) {}
                ...
                if (LabeledGroups.sortedKeys && LabeledGroups.sortedKeys.length > 0) {
                    ...
                    labelsLabel.text =
                        'Labels:'; // Newlines don't work in <option>, so just use plain text
                ...
                // Handle selection
                    ...
                    // Replace previous label-search selection highlight (do not touch periphery).
                    try { __HL3D.clearSource('ctrlD'); } catch (e) {}
                    try { highlightTextByStnums(new Set(), true, 'ctrlD'); } catch (e) {}

                    if (!selectedLabel) return;

                    const stnumsToHighlight = new Set();

                    for (let i = 0; i < NODES.length; i++) {
                        const n = NODES[i];
                        if (!n || !('attachmentLabel' in n)) continue;

                        let matches = false;
                        if (n.attachmentLabel && n.attachmentLabel.includes(selectedLabel)) {
                            matches = true;
                        } else if (selectedLabel in LabeledGroups.groupedLabels) {
                            if (LabeledGroups.groupedLabels[selectedLabel].some((groupLabel) => n.attachmentLabel.includes(groupLabel))) {
                                matches = true;
                            }
                        }
                        ...
                        try { __HL3D.add(n, 'ctrlD', __hlMat(n, 'ctrlD')); } catch (e) { n.material = __hlMat(n, 'selectCanvas'); }
                        if (Number.isFinite(n.stnum)) stnumsToHighlight.add(n.stnum);
                    }

                    try { highlightTextByStnums(stnumsToHighlight, false, 'ctrlD'); } catch (e) {}
                };
            }, 300);
        }
        ...
        if (canvasClicked && (event.key === 'f' && (event.ctrlKey || event.metaKey))) {
            ...
            setTimeout(function() {
                ...
                // Replace previous Ctrl+F highlight (do not touch periphery / selection layers).
                try { __HL3D.clearSource('ctrlF'); } catch (e) {}
                try { highlightTextByStnums(new Set(), true, 'ctrlF'); } catch (e) {}
                ...
                if (c_was_pressed) {
                    for (let i of NODES) {
                        ...
                        try { __HL3D.setBase(i, i.material); } catch (e) {}
                        if (('is_arrow' in i) || i.type == 0)
                            i.visible = false;
                    }
                } else {
                    if (rowNumber != -1000) {
                        // Clear previous ctrl+f selection/search highlight (do not touch periphery).
                        try { __HL3D.clearSource('ctrlF'); } catch (e) {}
                        try { highlightTextByStnums(new Set(), true, 'ctrlF'); } catch (e) {}
                    }
                }
                    ...
                    if (rr.length == 1) {
                        ...
                        for (let i = 0; i < NODES.length; i++) {
                            if (NODES[i].row[0] == rowNumber) {
                                try { __HL3D.add(NODES[i], 'ctrlF', __hlMat(NODES[i], 'ctrlF')); } catch (e) { NODES[i].material = __hlMat(NODES[i], 'selectCanvas'); }
                                if (Number.isFinite(NODES[i].stnum)) stnumsToHighlight.add(NODES[i].stnum);
                            }
                        }
                    } else if (rr.length == 2) {
                        if (rn.split(',')[0].trim()==='')
                ...
                try { highlightTextByStnums(stnumsToHighlight, false, 'ctrlF'); } catch (e) {}
                }
            }, 3000);
        }
    }
        ...
        document.addEventListener('clickText', (event) => {
            ...
            setTimeout(function () {
                if (reset) {
                    ...
                    try { __HL3D.clearSource('selectText'); } catch (e) {}
                ...
                for (const node of matchingNodes) {
                    if (stnum === -1 && node && typeof node.stnum !== 'undefined') stnum = node.stnum;
                    try { __HL3D.add(node, 'selectText', __hlMat(node, 'selectText')); } catch (e) { node.material = __hlMat(node, 'selectText'); }
                }
    ...
    function handleKeyUp(event) {
        if (canvasClicked && event.key === 'Escape') {
            ...
            if (!(event.ctrlKey || event.metaKey)) {
                // Full visual reset: clear all highlight layers so base colors return.
                try { __HL3D.clearAll(); } catch (e) {}
                c_was_pressed = false;
                for (let i = 0; i < NODES.length; i++) {
                    NODES[i].material = originalMaterials[i];
                    NODES[i].visible = true;
                }
                // Update base materials for highlight layering (post-Escape)
                try {
                    for (let i = 0; i < NODES.length; i++) { __HL3D.setBase(NODES[i], NODES[i].material); }
                    for (let i = 0; i < NODEShidden.length; i++) { __HL3D.setBase(NODEShidden[i], NODEShidden[i].material); }
                } catch (e) {}
